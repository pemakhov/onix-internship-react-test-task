...
export class CurrentStage extends React.Component {
  stages = [
      ...
      task: (
        <>
          <p>
            ...
            вложенности. Добавить и убрать элемент из объекта, выбрать элемент
            объекта по ключу. Переписать часть объекта с помощью spread
            оператора
          </p>
        </>
      ),
      ...
      task: (
        <>
          <p>
            ...
            биографии из прошлого задания таким образом чтобы все необходимое
            приходило в качестве параметров (без использования внешних
            переменных), а функция возвращала новый объект (а не перезаписывала
            старый). После вызова функции вывести в консоль старый и новый
            обьекты
          </p>
        </>
      ),
    },
    {
      ...
      goal: "ознакомиться с понятием области видимости, ознакомиться с fetch",
      task: (
        <>
          <p>
    ...
    {
      topic: "основы js, события",
      goal: "ознакомиться с событиями в js и react",
      task: (
        <>
          <p>
            сделать набор горячих клавиш для подсвечивания активного элемента
            списка из апи или урока 5. По клику делать элемент
            активным(подсвечивать бордером или менять цвет шрифта) или
            неактивным. Сделать перетаскивание элементов списка мышью чтобы
            можно было менять порядок вручную. Вывести на странице картинку с
            алертом который сработает на событие load и error.
          </p>
        </>
      ),
    },
  ...
  constructor(props) {
    ...
    this.state = {
      stages: this.stages,
      meetingDayNumber: 6,
      currentStageNumber: this.stages.length - 1,
    };
  }
